...
Steigung1, yAbschnitt1, err1 = some.linReg(x=x1*1e3, y=pulse1, p=x1[16:19], q=pulse1[16:19], x_name=r"$x_1 / \si{\milli\meter}$", y_name=r"$\frac{N}{\SI{120}{\second}}$", num=1,  x_add=1, file_name="build/plota.pdf")
...
EnergiemitReichw1 = 12.7e6

Steigung2, yAbschnitt2, err2 = some.linReg(x=x2*1e3, y=pulse2, p=x2[16:19]*1e3, q=pulse2[16:19], x_name=r"$x_2 / \si{\milli\meter}$", y_name=r"$\frac{N}{\SI{120}{\second}}$", num=3,  x_add=1, file_name="build/plotc.pdf")
Steigung2b, yAbschnitt2b, err2b = some.linReg(x=x2*1e3, y=max2*1e-6, p=x2[:]*1e3, q=max2[:]*1e6, x_name=r"$x_2 / \si{\meter}$", y_name=r"$E / \si{\electronvolt}$", num=4,  x_add=1, file_name="build/plotd.pdf")

steigungerr2 = ufloat(Steigung2, err2)
mitReichw2 = (1/2 * pulse2[0] - yAbschnitt2)/steigungerr2
EnergiemitReichw2 = 1234

some.tabelle([x1*1e3, pulse1, max1*1e-6], finished_file="build/tab1.tex", vars_name=[r"$x_1 / \si{\milli\meter}$", r"$# \text{Pulse}$", r"$E /\si{\mega\electronvolt} $"], label_text="tab1", caption_text=r"Die Reichweite $x_1$, die Anzahl der Impulse und die Position des Maximums.", precision=2) 
some.tabelle([x2*1e3, pulse2, max2*1e-6], finished_file="build/tab2.tex", vars_name=[r"$x_2 / \si{\milli\meter}$", r"$# \text{Pulse}$", r"$E / \si{\mega\electronvolt}$"], label_text="tab2", caption_text=r"Die Reichweite $x_2$, die Anzahl der Impulse und die Position des Maximums.", precision=2) 
...
#save solution
file = open("build/solutionneu.txt", "w")
