...
#V502 Messung
index6, I1= some.neueWerte(file_name="data/datac1.txt", finished_file="build/tabc1.tex",  vars_name=[r"Index", r"$I / \si{\ampere}$"], label_text="tabc1", caption_text=r"Die Indexwerte entsprechen der Höhe bei dem jeweiligen Strom und der Beschleunigungsspannung $U_\text{B} = \SI{250}{\volt}$." , precision=3)
index7, I2= some.neueWerte(file_name="data/datac2.txt", finished_file="build/tabc2.tex",  vars_name=[r"Index", r"$I / \si{\ampere}$"], label_text="tabc2", caption_text=r"Die Indexwerte entsprechen der Höhe bei dem jeweiligen Strom und der Beschleunigungsspannung $U_\text{B} = \SI{360}{\volt}$." , precision=3)
#V502 Erdmagnetfeld 
I_hor = 0.19 #Ampere
phi = 79.5 #Grad
N = 20 #Windungen 
...
D_Uderr = unp.uarray(D_Ud, [err1, err2, err3, err4, err5])
some.tabelle([1/U_B*1e3, D_Uderr*1e3], finished_file="build/tab1b.tex", vars_name=[r"$\frac{1}{U_\text{B}} / \si[per-mode=fraction]{\per\kilo\volt}$", r"$\frac{D}{U_\text{d}} / \si{\kilo\meter\per\volt}$"], label_text="tab1b", caption_text=r"Die Inverse der Spannung $1/U_\text{B}$ und die Empfindlichkeit $D/U_\text{d}$.", precision=2) 
...
#e/m 
steigung = np.array([Steigung6, Steigung7])
