...
U2 ,N2, I2 = some.neueWerte(file_name="data/datab.txt", finished_file="build/tabb.tex",  vars_name=[r"$U / \si{\volt}$", r"$\frac{N}{\SI{130}{\second}}$", r"$I / \si{\micro\ampere}$"], label_text="tabb", caption_text=r"Die angelegte Spannung des elektrischen Feldes innerhalb des Geiger-Müller-Zählrohrs, die Anzahl der jeweils gemessenen Impulse und der Strom innerhalb des Geiger-Müller-Zählrohrs." , precision=2)
...
steigung1, yabschnitt1, err = some.linReg(x=U1, y=N, yerr=errN, p=U1[3:15], q=N[3:15], x_name=r"$U / \si{\volt}$", y_name=r"$N / \frac{1}{\si{\second}}$", num=1,  x_add=30, file_name="build/plot1.pdf")

steigung1 = ufloat(steigung1, err[0])
yabschnitt1 = ufloat(yabschnitt1, err[1])
...
deltaQ2value = unp.nominal_values(deltaQ2e)
some.tabelle([U2, I2*1e6, N2, deltaQ2*1e9, deltaQ2e/1e10], finished_file="build/tab2.tex", vars_name=[r"U / \si{\volt}", r"I / \si{\micro\ampere}", r"N/second", r"$\Delta Q * 10^-9 C$", r"$\Delta Q *10^10 e0$"], label_text="tab1", caption_text=r"Die Spannung, die Stromstärke, die Anzahl der Impulse, die transportierte Ladungsmenge und die transporte Ladungsmenge in Einheiten der Elementarladung.", precision=2) 
some.plot(U2, deltaQ2value/1e10, x_name=r"$U / \si{\volt}$", y_name=r"$\Delta Q / \cdot 10^{10} \si{\elementarycharge}$", num=2, file_name="build/plot2.pdf")
