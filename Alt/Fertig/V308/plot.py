...
def allgB1(x, I, R, p):
    return ((4*np.pi*1e-7*I)/2)*(R**2/((R**2+(x-p)**2)**(3/2)))*100

def allgB2(x, mu_0, I, R, n, p, q):
    return ((mu_0*I)/2)*(R**2/((R**2+(x-p)**2)**(3/2)))*n+((mu_0*I)/2)*(R**2/((R**2+(x-q)**2)**(3/2)))*n

def allgB21(x, I, R, p, q):
    return ((4*np.pi*1e-7*I)/2)*(R**2/((R**2+(x-p)**2)**(3/2)))*100+((mu_0*I)/2)*(R**2/((R**2+(x-q)**2)**(3/2)))*100
...
paramkurz, pcovkurz2 = curve_fit(allgB1, x1kurz, B1kurz, p0=[1, 2.05e-2, 5e-2])
...
paramHelm1, pcovHelm12 = curve_fit(allgB21, x2r, B2r, p0=[4, 6.25e-2, 1e-2, 1e-2])
errHelm1 = np.sqrt(np.abs(np.diag(pcovHelm12)))
parametersHelm2, pcovHelm2 = curve_fit(allgB2, x2d*1e2, B2d*1e3)
paramHelm2, pcovHelm22 = curve_fit(allgB21, x2d, B2d, p0=[4, 6.25e-2, 9e-2, -2e-2])
errHelm2= np.sqrt(np.abs(np.diag(pcovHelm22)))
parametersHelm3, pcovHelm3 = curve_fit(allgB2, x23*1e2, B23*1e3)
paramHelm3, pcovHelm32= curve_fit(allgB21, x23, B23, p0=[3, 6.25e-2, 10e-2, -2.5e-2])
