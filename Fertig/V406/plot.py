...
def func2(phi, b, a, d):
    return 2* np.cos((np.pi* a*np.sin(phi))/(welle)) *(welle / (np.pi * b * np.sin(phi))) * np.sin((np.pi * b * np.sin(phi))/welle) +d

def func3(phi, b, a, d, c):
...
params3, err3 = some.curvefit(x= phi3, y=np.sqrt(I3), num=3, x_add=0, function= func3, x_name=r"$\varphi / \si{\radian}$", y_name=r"I / \si{\ampere}", file_name="build/plot3.pdf", p0=[spaltbreite, spaltbreite, d, c])

params4, pcov4 = curve_fit(func3, phi3, np.sqrt(I3), p0=[doppelspalt, spaltbreite, d, c])
err4= np.sqrt(np.diag(pcov4))
newphi3= np.linspace(phi3[0], phi3[-1], 10000)

plt.figure(4)
plt.plot(phi3, I3, "xr", label=r"Daten")
plt.plot(newphi3, func3(newphi3, *params4)**2, "k", label="Fit", linewidth=1.0)
plt.plot(newphi3, func3(newphi3, 5.5e-5, 48e-5, d, 7.0e-5)**2, "g", label="Theoriekurve", linewidth=1.0)
