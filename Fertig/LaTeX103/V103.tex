...
\label{fig:abb1}
\end{figure}
\noindent Die Durchbiegung $D(x)$ bezeichnet die Verschiebung eines Oberflächenpunktes
...
Wird \eqref{eqn:Momente2} integriert und die Gleichung
nach $D(x)$ umgestellt, ergibt sich für die Biegung bei einseitiger Einspannung
\begin{equation}
...
eine Kraft angreifen (siehe Abbildung 2), greift an der Querschnittsfläche die Kraft
...
\label{eqn:rechts}
\end{equation}
Werden beide beide Gleichungen integriert, ergibt sich 
\begin{equation*}
...
Die Apparatur ist in Abbildung 3 zu sehen. %Abbildung Apparatur erwähnen
...
angehängtes Gewicht gemessen. Danach wird an das Stabende ein Gewicht angehängt (siehe Abbildung 1).
...
mit Gewicht gemessen (siehe Abbildung 2). Hier wird für die erste Hälfte des Stabes $\frac{L}{2} \leq x \leq L$
